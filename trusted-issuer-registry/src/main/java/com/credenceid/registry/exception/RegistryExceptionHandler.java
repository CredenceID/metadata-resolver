...
import com.credenceid.registry.dto.Error;
...
public class RegistryExceptionHandler extends ResponseEntityExceptionHandler {
    private static final Logger exceptionHandlerLogger = LoggerFactory.getLogger(RegistryExceptionHandler.class);

    @ExceptionHandler(value = {IssuerNotFoundException.class})
    public ResponseEntity<Error> handleIssuerNotFoundException(
            IssuerNotFoundException ex, WebRequest request) {
        exceptionHandlerLogger.error(ex.getMessage());
        var error = new Error("103", ex.getMessage());
